...
**The gate measures the installed composition path.** A throughput estimate that omitted daemon
synchronization, frame queuing, notification, and the writer thread reported 190 MB/s, 2.4× above
the measured path. A parser-only frontend estimate reported 110 MB/s, about 25× the real frontend
acknowledgement rate once rendering and IPC are included. Neither estimate is acceptance evidence.
The gate runs the daemon and frontend together and measures that path directly.
