...
the mirror keeps pace with the independently measured detached daemon tee demand. The provider
owner floor is a fixed **80 MB/s**, and installed-system tests separately require zero gap and
final marker delivery through the complete composition path.
...
(the installed composition gate and `cargo test --release --test demand -- --ignored`), never
