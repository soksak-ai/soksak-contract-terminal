...
fn benchmark_report_is_versioned_json() {
    let report = Report {
        unit: "vt100".into(), feed_mb_s: 120.5, rehydrate_ms: 1.2, paint_bytes: 4096,
        cold_ms: 2.3, cold_bytes: 2048, live_bytes: 1024, rss_bytes: 8192,
        demand_mb_s: 75.0, gap_bytes: 0, tail_seen: true,
    };
    let json = report.to_json();
    assert!(json.contains(BENCHMARK_REPORT_SPEC));
    let parsed = Report::from_json(&json).expect("parse benchmark JSON");
    assert_eq!(parsed.unit, "vt100");
