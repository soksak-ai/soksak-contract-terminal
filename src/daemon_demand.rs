...
//!
//! [`crate::demand`] 는 §6.2 의 배선을 흉내 내 관의 **용량**을 쟀다. 용량은 요구가 아니다:
//! 미러가 마주하는 것은 관이 낼 수 있는 최대가 아니라 데몬이 실제로 보내는 양이고, 그것을 정하는
//! 것은 **데몬의 읽기 루프를 무엇이 늦추느냐**다. 그 답이 두 모드에서 다르다 — 그러니 두 모드를
//! 다 재야 한다. 모델은 그 차이를 볼 수 없다(모델에는 프론트가 없다).
//!
//! ## 두 모드
//!
